...
from utils.weight_drop import WeightDrop
from ON_LSTM import ONLSTMStack

@register_model('onlstm_encoder')
class OnlstmEncoder(FairseqEncoder):
    ...
    @classmethod
    def build_model(cls, parameter_list):
        pass
    ...
    def __init__(self, rnn_type, ntoken, ninp, nhid, chunk_size, nlayers, dropout=0.5, dropouth=0.5, dropouti=0.5, dropoute=0.1, wdrop=0, tie_weights=False):
        ...
        if tie_weights:
            ...
            self.decoder.weight = self.encoder.weight # decoder,encoder共用embedding matrix,这里encoder和decoder都是embedding layer
    ...
    def forward(self, input, hidden, return_h=False):
        emb = embedded_dropout(
            self.encoder, input,
            dropout=self.dropoute if self.training else 0 # 手写了dropout layer
        )
