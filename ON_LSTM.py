...
class ONLSTMCell(nn.Module):
    ...
    def forward(self, input, hidden,
        ...
        if transformed_input is None:
            transformed_input = self.ih(input) # (b,l,4*h+2*nc)

        gates = transformed_input + self.hh(hx) # (b,l,4*h+2*nc) 一次计算了所有gates
        cingate, cforgetgate = gates[:, :self.n_chunk*2].chunk(2, 1) # (b,l,nc)
        outgate, cell, ingate, forgetgate = gates[:,self.n_chunk*2:].view(-1, self.n_chunk*4, self.chunk_size).chunk(4,1) # (b,l,c)
        ...
        cell = F.tanh(cell) # cell是new hidden state
        outgate = F.sigmoid(outgate)

        # cy = cforgetgate * forgetgate * cx + cingate * ingate * cell

        overlap = cforgetgate * cingate
        forgetgate = forgetgate * overlap + (cforgetgate - overlap) # 由forget gate和input gate控制overlap
    ...
    def sample_masks(self):
        for m in self.drop_weight_modules: # dropout
            m.sample_mask()
...
class ONLSTMStack(nn.Module):
    ...
    def forward(self, input, hidden):
        ...
        for l in range(len(self.cells)):
            curr_layer = [None] * length # length==seq_len
            dist = [None] * length
            t_input = self.cells[l].ih(prev_layer) # 每个step的input
            ...
            prev_layer = torch.stack(curr_layer) # 输入到下一个layer的Input
            dist_cforget, dist_cin = zip(*dist) # distance in a layer
